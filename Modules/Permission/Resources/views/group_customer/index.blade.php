                                    ...
                                    <td>
                                        @can('statusBtn', \App\Models\Permission\AuthRule::class)
                                            @if($data->status==\App\Models\Permission\AuthGroup::ENABLED)
                                                <button type="button" class="btn btn-danger" onclick="changeStatus('{{route('permission.group_customer.update', ['id'=>$data->id])}}', this.value);" value="{{$data->status}}">禁用</button>
                                            @else
                                                <button type="button" class="btn btn-success" onclick="changeStatus('{{route('permission.group_customer.update', ['id'=>$data->id])}}', this.value);" value="{{$data->status}}">启用</button>
                                            @endif
                                        @endcan
                                        @can('editBtn', \App\Models\Permission\AuthRule::class)
                                                <button type="button" class="btn btn-info" onclick="edit('{{route('permission.group_customer.edit',['id'=>$data->id])}}')">编辑</button>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
...
<script>
    function create()
    {
        layer.open({
            ...
            shadeClose: true,
            title :'新增',
            content: '{{route('permission.group_customer.create')}}',
            shift:-1,
