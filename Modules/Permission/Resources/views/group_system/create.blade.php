            ...
            <form id="my_form" class="form-horizontal" method="post" enctype="multipart/form-data">
                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" name="cn_name" value="">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 col-sm-offset-1 control-label">角色英文名：</label>
                        <div class="col-sm-6">
                            <input type="text" class="form-control" name="name" value="" placeholder="英文字母或_组合">
                        </div>
                    </div>
                    <div class="form-group">
    ...
    <script>
        $(function () {
            ...
            $('#menus_rule_tree').jstree({
                "core": {
                    ...
                    "data": {
                        'url': '{{route('permission.permission.get_menus_rule_tree',['group_id'=>0])}}',
                        'data': function (node) {
                            return node;
                        }
                    },
        ...
        function confirmCreate()
        {
            try {
                ...
                $.ajax({
                    url: "{{route("permission.group_system.store")}}",
                    // traditional: true,
                    method: "post",
                    data: params,
        ...
        function changeHandle(node)
        {
            ...
            let name_customer_admin = '{{\App\Models\Permission\AuthGroup::NAME_CUSTOMER_ADMIN}}';
            let name_node = $('input[name=name]');
            name_node.val('');
            name_node.attr('readonly', false);
            let select_val = $(node).val();
            if (select_val.length > 0) {
                if (internal_customer_id === select_val) {
                    name_node.attr('readonly', false);
                } else {
                    name_node.val(name_customer_admin);
