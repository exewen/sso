    ...
    <section class="content">
        <div class="box box-primary" style="padding:20px 0;">
            <form id="my_form" class="form-horizontal" method="post" enctype="multipart/form-data">
                ...
                <input type="hidden" name="status" value="{{\App\Models\Permission\User::ENABLED}}">
                <div class="box-body">
                    <div class="form-group">
                        <label class="col-sm-2 col-sm-offset-1 control-label">客户：</label>
                        <div class="col-sm-6">
                            <select class="form-control select2" name="customer_id"  onchange="customerdChange(this)">
                                <option value="" title=" ">请选择</option>
                                @foreach($customers as $v)
                                    <option value="{{$v['id']}}" title=" ">{{$v['name']}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            {!! Timezone::selectForm(null,'时区',['class'=>'form-control select2','name'=>'timezone'],['title'=>' ']) !!}
                        </div>
                    </div>
                    <div class="form-group">
...
@section('scripts')
    ...
    <script>
        ...
        let authGroupsJson = '{!! json_encode($authGroups) !!}';// 全部角色数据
        /**
         * 展示菜单权限列表
         */
        ...
        function customerdChange(node) {
            let select_val = $(node).val();
            let node_next = $('select[name=group_id]');
            if (select_val.length > 0) {
                emptySelectNode(node_next);
                $.each(JSON.parse(authGroupsJson), function (i, v) {
                    if (v.customer_id == select_val) {
                        node_next.append('<option value="' + v.id + '" title=" ">' + v.cn_name + '</option>');
                    }
                });
            } else {
                emptySelectNode(node_next);
        ...
        function confirmCreate()
        {
            try {
                ...
                $(params).each(function () {
                    ...
                    // if (this.name === 'password' || this.name === 'password_2') {
                ...
                $.ajax({
                    url: "{{route("permission.user_system.store")}}",
                    // traditional: true,
                    method: "post",
                    data: params,
